    ...
    <div v-if="report" class="space-y-4">
      ...
      <div class="bg-white rounded p-3 border">
        ...
        <div class="flex items-center gap-4 text-xs">
          <div class="flex items-center gap-1 text-green-600">
            ...
            {{ report.approve_votes || 0 }} {{ t('vote.approve') }}
          </div>
          <div class="flex items-center gap-1 text-red-600">
            <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 24 24">
              <path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/>
            </svg>
            {{ report.refute_votes || 0 }} {{ t('vote.refute') }}
          </div>
          <div class="flex items-center gap-1 text-gray-600">
            <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 24 24">
              <path d="M12 2C13.1 2 14 2.9 14 4C14 5.1 13.1 6 12 6C10.9 6 10 5.1 10 4C10 2.9 10.9 2 12 2Z"/>
            </svg>
            {{ report.not_ai_votes || 0 }} {{ t('vote.notAi') }}
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-3 gap-2">
        ...
        >
          ...
          <span class="text-xs font-medium">{{ t('vote.approve') }}</span>
        </button>
        ...
        >
          ...
          <span class="text-xs font-medium">{{ t('vote.refute') }}</span>
        </button>
        ...
        >
          ...
          <span class="text-xs font-medium">{{ t('vote.notAi') }}</span>
        </button>
      </div>

      <!-- Vote explanations -->
      <div class="text-xs text-gray-500 space-y-1">
        <div><strong>{{ t('vote.approve') }}:</strong> {{ t('vote.approveHelp') }}</div>
        <div><strong>{{ t('vote.refute') }}:</strong> {{ t('vote.refuteHelp') }}</div>
        <div><strong>{{ t('vote.notAi') }}:</strong> {{ t('vote.notAiHelp') }}</div>
      </div>

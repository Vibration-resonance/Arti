<template>
  <div class="bg-gray-50 rounded-lg p-4 border">
    ...
    <div v-else-if="status" class="space-y-3">
      ...
      <!-- Reports summary -->
      <div v-if="status.reports.length > 0" class="mt-3">
        <div class="text-sm text-gray-600 mb-2">
          {{ t('popup.reportsCount', { count: status.reports.length }) }}
        </div>
        ...
        <div class="bg-white rounded p-3 border">
          <div class="flex items-center justify-between mb-2">
            <span class="text-sm font-medium text-gray-800">
              {{ t(`reportType.${status.reports[0].report_type}`) }}
            </span>
      ...
      <div v-if="status.domain_reports_count > 0" class="mt-3 p-3 bg-orange-50 border border-orange-200 rounded">
        <div class="flex items-center gap-2 text-orange-700">
          ...
          <span class="text-sm font-medium">
            {{ t('popup.domainReports', { count: status.domain_reports_count }) }}
          </span>
        </div>
      </div>
    </div>
...
const getStatusDescription = (status: PageStatus) => {
  if (status.status === 'unknown' && status.domain_reports_count > 0) {
    return t('status.unknownWithReports')
  }

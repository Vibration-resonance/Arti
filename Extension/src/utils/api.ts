...
class ApiClient {  private async getHeaders(): Promise<HeadersInit> {
  ...
  async getPageStatus(url: string): Promise<ApiResponse<any>> {
    ...
    return this.get(`/getPageStatus?url=${encodedUrl}`);
  }
  ...
  async createReport(data: {
    ...
    return this.post('/create-report', data);
  }
  async createVote(data: {
    report_id: string;
    vote_type: 'approve' | 'refute' | 'not_ai';
  }): Promise<ApiResponse<any>> {
    return this.post('/createVote', data);
  }
  ...
  async getTopUsers(params?: {
    ...
    return this.get(`/getTopUsers${queryString ? `?${queryString}` : ''}`);
  }
  async getRecentReports(params?: {
    ...
    const queryString = searchParams.toString();
    return this.get(`/getRecentReports${queryString ? `?${queryString}` : ''}`);
  }

  async getWhitelistDomains(): Promise<ApiResponse<any>> {
    return this.get('/getWhitelistDomains');
  }
  async createCheckoutSession(data: {
    plan: 'Premium' | 'Pro';
    return_url?: string;
  }): Promise<ApiResponse<any>> {
    return this.post('/createCheckoutSession', data);
  }
}

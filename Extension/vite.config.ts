    ...
    rollupOptions: {
      ...
      output: {
        entryFileNames: (chunkInfo) => {
          // Pour le background script, on veut un seul fichier sans imports
        ...
        // Configuration spéciale pour le background script
        manualChunks: (id) => {
          // Forcer le background script à être dans un seul chunk
          if (id.includes('background.ts')) {
            return 'background';
          }
          // Forcer les utilitaires utilisés par le background dans le même chunk
          if (id.includes('src/utils/') && (id.includes('api.ts') || id.includes('helpers.ts'))) {
            return null; // Les inclure inline dans le background
          }
        }
      }
    },
